...
export class PersistentReactiveDict extends ReactiveDict {
    ...
    load() {
        var psList, psaList;
        const PSKEYS = "__" + this.dictName + "_PSKEYS__";
        const PSAKEYS = "__" + this.dictName + "_PSAKEYS__";
        psList = amplify.store(PSKEYS);
        // console.log(`load ${PSKEYS} ${typeof psList}`, psList);
        if (psList != null && typeof psList === 'object' && psList.length !== void 0) {
        ...
        psaList = amplify.store(PSAKEYS);
        // console.log(`load ${PSAKEYS} ${typeof psaList}`, psaList);

        if (psaList != null && typeof psaList === 'object' && psaList.length !== void 0) {
    ...
    store(type, key, value) {
        ...
        if (type === 'get') {
            ...
            // console.log(`store ${type} ${key},${value}`, this.psKeyList, this.psaKeyList);
    ...
    dec(key, a = 1, persist, auth) {
        var val = parseInt(super.get.call(this, key));
        this.set(key, value - parseInt(a), persist, auth);
    }
    ...
    setDefault(key, value, persist, auth) {
        ...
        // else {
        //     console.info(`setDefault ${key} already exists`, s)
        // }
    }
...
// _.extend(Session, new PersistentReactiveDictClass("session"));
